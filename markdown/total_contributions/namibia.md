...
The `public contributions` and `private contributions` by users in Namibia on `2021/9/30 1:24 PM UTC`. This list contains users from `Namibia` and cities `Windhoek` `Swakopmund` `Walvis-bay` `Otjiwarongo`.

There are `137 countries` and `674 cities` can be found [here](https://github.com/gayanvoice/top-github-users).

There are `468 users`  in Namibia. You need at least `0 followers` to be on this list.
