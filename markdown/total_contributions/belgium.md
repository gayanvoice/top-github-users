...
The `public contributions` and `private contributions` by users in Belgium on `2021/6/14 6:40 AM UTC`. This list contains users from `Belgium` and cities `Antwerp` `Brussels` `Ghent` `Bruges` `Leuven` `Liège` `Namur`.

There are `71 countries/states` and `439 cities` can be found [here](https://github.com/gayanvoice/top-github-users).

There are `200 users`  in Belgium. You need at least `106 followers` to be on this list.
